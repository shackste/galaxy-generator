...
def train_biggan(batch_size: int = 1, epochs: int = 2, N_batches_evaluate: int = 2):
    ...
    for epoch in trange(epochs, desc="epochs"):
        ...
        for images_valid, labels_valid in data_loader_valid:
            break
        evaluate(images_train, labels_train, images_valid, labels_valid, track=track)
...
def evaluate(images_train: torch.Tensor, labels_train: torch.Tensor, images_valid: torch.Tensor, labels_valid: torch.Tensor, track=False) -> None:
...
if __name__ == "__main__":
    if track:
        train_biggan_tracked(batch_size=batch_size, epochs=epochs)
    else:
        train_biggan(batch_size=batch_size, epochs=epochs)