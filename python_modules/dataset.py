...
class MakeDataLoader:
    def __init__(self, test_size=0.1, random_state=2, device=torch.device("cpu")):
        self.device = device
        dataset = DataSet()
    ...
    def get_data_loader_train(self, batch_size=64):
        return DataLoader(self.dataset_train, batch_size=batch_size, shuffle=True, drop_last=True, pin_memory=self.device==torch.device("cuda"))

    def get_data_loader_test(self, batch_size=64):
        return DataLoader(self.dataset_test, batch_size=batch_size, shuffle=True, drop_last=True, pin_memory=self.device==torch.device("cuda"))